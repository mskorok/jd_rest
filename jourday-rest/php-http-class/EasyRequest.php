...
class EasyRequest
{
    ...
    public function send()
    {
        ...
        if (empty($request['uri'])) {
            throw new \RuntimeException('Request URI cannot be empty.');
        }
    ...
    protected function sendWithCurl(array $request)
    {
        ...
        if (!empty($this->options['post_file'])) {
            $curlOptions = array_merge($curlOptions, [CURLOPT_POSTFIELDS => $this->options['post_file']]);
        }
     ...
     * @return array
     */
    protected function parseStringParams($queryString, array &$array = array()): array
    {
    ...
    protected function getHandler()
    {
        ...
        if ($this->options['handler'] !== null) {
            if (empty($available[$this->options['handler']])) {
                throw new \RuntimeException(sprintf('Handler "%s" is not available.', $this->options['handler']));
            }
        ...
        throw new \RuntimeException('Have no available handler based on your request options/ PHP config.');
    }
}
