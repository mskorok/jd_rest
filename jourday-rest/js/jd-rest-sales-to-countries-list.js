...
app.controller('listCtr', function($scope) {
    ...
    var showUrl = rest_admin_sales_to_countries_show;
    var getUrl = 'sales-responsible-to-countries?include=Country,User';
    var collectionKey = 'salesResponsibleToCountries';

    var jd_rest_admin_list = {
        list: function () {
            var url = jd_rest_host + '?action=jourday_ajax_form_get&method=GET&url=' + getUrl + '%26offset=' + jd_rest_admin_offset;
                            ...
                            if (els) {
                                ...
                                [].forEach.call(els, function (el) {
                                    html += '<tr>' +
                                        ...
                                        '<td class="id"><a href="' + showUrl + '?el_id=' + el.id + '" target="_blank">' + el.User.firstName + ' ' + el.User.lastName + '</a></td>' +
                                        '<td class="id"><a href="' + showUrl + '?el_id=' + el.id + '" target="_blank">' + el.Country.name + '</a></td>' +
                                        '</tr>';
                                });
