...
app.controller('reportCtr', function($scope) {
    ...
    var showUrl = jd_payable_url;

    var collectionKey = 'revenuePayable';
    var pdfKey = 'revenuePayableWithPdf';
    var excelKey = 'revenuePayableWithExcel';

    var uri = {
        'Admin': {
            data:'/widget/admin/payable',
            pdf: '/widget/admin/payable/pdf',
            excel: '/widget/admin/payable/excel'
        },
        'JourdayProvider': {
            data:'/widget/jourday-provider/payable',
            pdf: '/widget/jourday-provider/payable/pdf',
            excel: '/widget/jourday-provider/payable/excel'
        },
        'JourdayBeneficiary': {
            data:'/widget/beneficiary/payable',
            pdf: '/widget/beneficiary/payable/pdf',
            excel: '/widget/beneficiary/payable/excel'
        },
        'ActivityProvider': {
            data:'/widget/provider/payable',
            pdf: '/widget/provider/payable/pdf',
            excel: '/widget/provider/payable/excel'
        },
        'CustomerSupport': {
            data:'',
            pdf: '',
                        ...
                        try {
                            ...
                            if (reports) {
                                ...
                                [].forEach.call(reports, function (report) {
                                    html += '<tr>' +
                                        '<td class="id"><a href="' + showUrl + '?user_id=' + report.userId + '" target="_blank">' + report.firstName + ' ' + report.lastName + '</a></td>' +
                                        '<td class="id"><a href="' + showUrl + '?user_id=' + report.providerId + '" target="_blank">' + report.providerId + '</a></td>' +
                                        '<td class="id"><a href="' + showUrl + '?user_id=' + report.providerId + '" target="_blank">' + report.providerType + '</a></td>' +
                                        '<td class="id"><a href="' + showUrl + '?product_id=' + report.productId + '" target="_blank">' + report.productName + '</a></td>' +
                                        '<td class="id">' + report.categoryName + '</td>' +
