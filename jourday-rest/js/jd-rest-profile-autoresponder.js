...
app.controller('createCtr', function($scope) {
    ...
    var checkUrl = 'users/has/autoresponder/';

    var rest_plugin_create = {
        formUrl: 'wp/autoresponder/create',
        createUrl: '/autoresponders/',
        init: function () {
            //
        },
        ...
        submit: function () {
            ...
            rest_plugin_edit.hideSelect(form);
            if (form) {
        ...
        send: function (form) {
            ...
            var url = jd_rest_host + '?action=jourday_ajax_send_form_post_models&url=' + rest_plugin_create.createUrl;
        ...
        create: function () {
            var url = jd_rest_host + '?action=jourday_ajax_form_get&method=GET&url=' + rest_plugin_create.formUrl;
            var xhr = new XMLHttpRequest();
            xhr.onload = function () {
                if (this.readyState === 4) {
                    if (this.status === 200) {
    ...
    var rest_plugin_edit = {
        getUrl: 'wp/autoresponder/edit/user/',
        editUrl: '/wp/form/create/main',
        model: 'Autoresponder',
        formId: 'Autoresponder_edit_form',
        mainField: -1,
        relatedModels: {},
        modelsWithImages: [],
        imageFields: {},
        ...
        hideSelect: function (form) {
            var user_id = form.user_id;
            
            var id = jd_get_authorized();
            if (user_id) {
                var options = user_id.options;
                var opt = null;
        ...
        addRemoveListeners: function (form) {
            ...
            if (form[this.mainField]) {
                if (this.modelsWithImages.indexOf(model) !== -1) {
                    rest_plugin_edit.removeModel(form.parentNode);
                } else {
                    rest_plugin_edit.removeModel(form.parentNode);
                }
                ...
                if (rel_settings.isImage) {
                    rest_plugin_edit.removeRelatedModel(form.parentNode);
                } else {
                    rest_plugin_edit.removeRelatedModel(form.parentNode);
                }
            } else {
        ...
        editMainModel: function (form) {
            ...
            var form_data = new FormData(form);
            var error_container, success_container;
            console.log('m', model);
            form_data.append('model', model);
                ...
                if (this.readyState === 4) {
                    if (this.status === 200) {
                        try {
                            ...
                            success_container = document.getElementById('jd_success_container');
                            error_container.innerHTML = '';
                            if (response.result === 'error') {
                                if (error_container) {
                                    var html = '';
                                ...
                                if (container) {
                                    ...
                                    self.hideSelect(new_form);
                                    jd_successEdit(new_form);
                                }
                                if (success_container) {
                                    success_container.textContent = 'Successfully updated';
                                    setTimeout(function () {
                                        success_container.textContent = '';
                                    }, 14000);
                                }
                            }
                        } catch (e) {
        ...
        edit: function () {
            var el_id = jd_get_authorized();
            if (!el_id) {
                window.location.href = redirectUrl;
            }


            var url = jd_rest_host + '?action=jourday_ajax_form_get&method=GET&url=' + rest_plugin_edit.getUrl + el_id;
            var xhr = new XMLHttpRequest();
            xhr.onload = function () {
                if (this.readyState === 4) {
                    if (this.status === 200) {
                        try {
                            var container = document.getElementById('plugin_edit_container');
                            ...
                            [].forEach.call(remove_buttons, function (del) {
                                if (del) {
                                    ...
                                    del.addEventListener('click', function () {
                                        rest_plugin_edit.addRemoveListeners(form);
                                    })
                                }
                            });
                            rest_plugin_edit.init();
                            rest_plugin_edit.autocomplete();
                            rest_plugin_edit.submit();
                        } catch (e) {
                            console.log('error', e);
                        }
                    } else {
    ...
    var rest_autoresponder = {
        check: function () {
            var el_id = jd_get_authorized();
            if (!el_id) {
                window.location.href = redirectUrl;
            }
            var url = jd_rest_host + '?action=jourday_ajax_form_get&method=GET&url=' + checkUrl + el_id;
            var xhr = new XMLHttpRequest();
            xhr.onload = function () {
                if (this.readyState === 4) {
                    if (this.status === 200) {
                        try {
                            var response = JSON.parse(this.response);
                            if (response.result === 'OK') {
                                rest_plugin_edit.edit();
                            } else {
                                rest_plugin_create.create();
                            }
                        } catch (e) {
                            console.log('error', e);
                        }
